...
| radius-sm / md / lg | `6 / 6 / 8` | same | `--rect-radius-*`, `--h5m-radius` |
...
- Radius scale: card 8, control 6. No pill buttons.
...
- Overview IA: hero first (uptime / memory used / storage used), then 系统 / 端口 / 网络, then the rest. Hero uptime drops seconds (poll is not 1s). Memory big number is occupancy (100 − 可用), not 可用 itself. Memory and storage cards are folded into the hero. 「隐藏」 still far right on remaining cards; bars stay `width: 100%`.
